...
Plugins are defined in the C<plugins> file in the C<DATADIR>. Each plugin is a
Perl source file in the C<PLUGINDIR> (typically, the `plugins` directory in the
revbank code git repository). Plugins are always iterated over in the order
